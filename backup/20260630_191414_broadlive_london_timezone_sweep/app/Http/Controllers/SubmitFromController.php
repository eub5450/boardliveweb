...
class SubmitFromController extends Controller
{
 ...
 $updatedCount = 0;
        ...
        $agency_code=Agency::orderby('id','desc')->first();
          if($request->hasFile('photo_id')){
            ...
            if($request->hasFile('selfie')){
                ...
                $sepfie_path = 'store/agency/';
                $selfie_url = $sepfie_path.$selfie_name;
                $selfie->move($sepfie_path, $selfie_name);
            }else{
                $selfie_url = 'store/profile/default.png';
            }
             if($request->hasFile('nid')){
                $nid = $request->file('nid');
                $nid_name = uniqid().'.'.strtolower($nid->getClientOriginalExtension());
                $nid_path = 'store/agency/';
                $nid_url = $nid_path.$nid_name;
                $nid->move($nid_path, $nid_name);
            }else{
                $nid_url = 'store/profile/default.png';
            }
            $agency=new Agency;
           $agency->user_id=$check_user->id;
           $agency->name=$request->agency_name;
           $agency->code=$agency_code->code+1;
  ...
  function addDurations($duration1, $duration2) {
    ...
    {
        return Carbon\Carbon::now(config('app.timezone', 'Asia/Dhaka'));
    }
