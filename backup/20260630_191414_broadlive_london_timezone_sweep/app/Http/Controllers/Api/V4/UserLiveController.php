...
use App\Services\V4\HomeService;
...
public function __construct(HomeService $service)
{
    ...
    private function handleHomeRequest(Request $request, string $type)
    {
        ...
        if ($type === 'home') {
            ...
            $data = [
                'lives_now' => $type === 'party'
                    ? $this->service->getLivesFast(1)
                    : $this->service->getFollowerLivesFast((int) $user->id),
            ];
        }
    ...
    public function Delete(Request $request)
    {
        ...
        if($check_day_time){
          ...
          $data->delete();
          $this->service->forgetLiveCaches((int) $id);
