...
$(document).ready(function() {
    $("#saven_winner .container .footer .footer_top .box_wrapper").click(function() {
        ...
        $.ajax({
            method: "get",
            url: ca + "fivestar/push_user_data",
            data: {
                bord_name: $(this).children("input").val(),
                bord_amount: $("#saven_winner .container .footer .footer_bottom .footer_bottom_right .images.active").children("input").val()
            },
            success: function(i) {}
        })
    })
...
Echo.channel("users_amount_name").listen(".users_amount_event", i => {
    ...
    if (n.play(), $(".this_is_users").addClass("active"), setTimeout(() => {
        ...
        var r = Number(Math.random() * 80).toFixed(0),
            a = Number(Math.random() * 80).toFixed(0),
            f = "";
