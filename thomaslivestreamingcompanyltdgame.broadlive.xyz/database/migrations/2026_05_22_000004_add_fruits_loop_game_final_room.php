...
class AddFruitsLoopGameFinalRoom extends Migration
{
    public function up()
    {
        ...
        if ($existing) {
            DB::table('bd_game_final_games')->where('id', $existing->id)->update([
                'name' => (string) ($cfg['name'] ?? 'Fruits Loop'),
                'is_active' => 1,
        ...
        DB::table('bd_game_final_settings')->updateOrInsert(
            ['game_id' => $gameId],
            [
                'bet_duration_sec' => (int) ($cfg['bet_duration_sec'] ?? 30),
    ...
    {
        $game = DB::table('bd_game_final_games')->where('game_code', 'fruits_loop')->first();
        if (!$game) {
            return;
        }
