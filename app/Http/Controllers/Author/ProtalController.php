...
class ProtalController extends Controller
{
    ...
    public function RecallStore(Request $request)
    {
        ...
        try {
            DB::transaction(function () use ($user, $portal, $settings, $amount, $request) {
            $user = User::where('id', $user->id)->lockForUpdate()->first();
            $portal = User::where('id', $portal->id)->where('country_id', (int) Auth::user()->country_id)->where('is_coin_protal_active', 1)->lockForUpdate()->first();
            ...
            return Redirect()->back()->with([
                'messege' => 'User, portal, or company user not found',
                'alert-type' => 'error',
            ]);
        }

        return Redirect()->route('country.author.protal-recall-list')->with([
            'messege' => 'Location recall successful with dynamic split',
            'alert-type' => 'success',
        ]);
    }
