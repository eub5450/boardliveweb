...
public function TimeCall(Request $request)
{
    ...
    if ($setting->game_status == 1 && $request->has('tray_id')) {
        ...
        while ($attempt < $maxAttempts) {
            try {
                return DB::transaction(function () use ($get_tray_id) {
                    $find = GradyTray::latest('id')->lockForUpdate()->first();
                    ...
                    if ($find && $find->status == 0) {
                        if ($find->tray_id >= $get_tray_id) {
                            ...
                            $datas = strtotime("+46 seconds", time());
                            $currentTime = Carbon::now()->timestamp;

                            // Check if tray_id already exists to prevent duplicate
                            $existing = GradyTray::where('tray_id', $datas)->lockForUpdate()->first();
                            if (!$existing) {
                                $gamestart = new GradyTray;
                                $gamestart->tray_id = $datas;
                                $gamestart->status = false;
                ...
                }, 1); // No automatic retry, we handle it manually
                ...
                if (str_contains($e->getMessage(), 'Deadlock found')) {
                    if ($attempt >= $maxAttempts) {
                        \Log::warning('Deadlock persisted after ' . $maxAttempts . ' attempts', [
                            'user_id' => $user->id,
                            'tray_id' => $get_tray_id
                        ]);
                  ...
                  $uniqueUserCount = GradyPots::where('tray_id', $find->tray_id)
                   ...
                   switch (true) {
                                ...
                                default:
                                    ...
                                    if (!in_array('lion', $check_winnerNames) && $lion_need_serve < $big_pots_balance && $uniqueUserCount > 2) {
