  ...
  public function Recall(Request $request)
    {
        ...
        $setting = $this->portalSettingRow();
        $portalPercentage = $this->portalSettingNonNegativeValue($setting, 'recall_portal_percentage', 70);
        $companyPercentage = $this->portalSettingNonNegativeValue($setting, 'recall_company_percentage', 30);
        $companyUserId = $this->portalSettingValue($setting, 'recall_company_user_id', 11111);
        ...
        try {
            DB::transaction(function () use ($request, $portalPercentage, $companyPercentage, $companyUserId) {
                $portal = User::where('id', $request->user_id)->where('is_coin_protal_active', 1)->lockForUpdate()->first();
                $companyUser = User::where('id', $companyUserId)->lockForUpdate()->first();

                if (!$portal || !$companyUser) {
                    throw new \RuntimeException('Portal or company user not found');
                }

                $amount = (float) $request->amount;
                $portalAmount = round(($amount * $portalPercentage) / 100, 2);
            ...
            return Redirect()->back()->with([
                'messege' => 'Portal or company user not found',
                'alert-type' => 'error'
            ]);
        }
