...

{
    public function FruitsControl()
    {
    	...
    	$data['game_serve_users_details']=FortunePots::orderby('id','desc')->whereNotIn('user_id', [559611, 559612, 559613])->limit(50)->get();
    ...
    public function TeenPattiThirdBalanceSetting(Request $request)
    {
        // Validate the request data
        $request->validate([
            'third_take_margin' => 'required|numeric',
        ...
        // Find the balance record by ID
        $balance = TeenPattiSetting::find(1);
    ...
    public function TeenPattiIDBlock(Request $request)
  {
    ...
    public function FruitsGameCommision(Request $request)
    {
        $data=FortuneSetting::find(1);
        $data->fruits_game_withdraw_parcentage=$request->fruits_game_withdraw_parcentage;
        $data->save();
         $notification=array(
                'messege'=>'Fruits Game Commission Update SuccessFully',
                'alert-type'=>'success'
            );
            return Redirect()->back()->with($notification);
    }
    public function GradyGameCommision(Request $request)
    {
        $data=GradySetting::find(1);
        $data->game_withdraw_parcentage=$request->game_withdraw_parcentage;
        $data->save();
         $notification=array(
                'messege'=>'Greedy Game Commission Update SuccessFully',
                'alert-type'=>'success'
            );
            return Redirect()->back()->with($notification);
    } 
    public function TeenPattiGameCommision(Request $request)
    {
        $data=TeenPattiSetting::find(1);
        $data->game_withdraw_parcentage=$request->game_withdraw_parcentage;
        $data->save();
         $notification=array(
                'messege'=>'TeenPatti Game Commission Update SuccessFully',
                'alert-type'=>'success'
            );
            return Redirect()->back()->with($notification);
    }
