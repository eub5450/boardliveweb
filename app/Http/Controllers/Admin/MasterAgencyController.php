...
class MasterAgencyController extends Controller
{
    ...
    {
    	$data = ChildAgency::where('child_agency_id',$id)->first();
    	$data->delete();
    	 $notification=array(
    ...
    {
        //return $request->all();
    	$check=ChildAgency::where('child_agency_id',$request->child_agency_id)->first();
    	if (!$check) {
    	$data=new ChildAgency;
    	$data->master_agency_id=$request->master_agency_id;
    	$data->child_agency_id=$request->child_agency_id;
    	$data->save();
    	$notification=array(
                'messege'=>'ChildAgency Added SuccessFully',
                'alert-type'=>'success'
            );
            return Redirect()->back()->with($notification);
    	}else{
    	    $check->delete();
    		$notification=array(
                'messege'=>'ChildAgency Have Already Another Master Agency',
                'alert-type'=>'error'
            );
            return Redirect()->back()->with($notification);
    	}
