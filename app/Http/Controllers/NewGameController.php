        ...
        if (FortuneTray::where('tray_id', $trayId)->exists()) {
            ...
            DB::transaction(function () use ($amount, $loginUser_one, $trayId) {
                $existingGameStart = FortunePots::where('tray_id', $trayId)
                 ...
                 $pusher = new Pusher\Pusher(
                  'b381e3e0d58fa42e2b76',
                  'dc40be3acd6a7ce84059',
                  '2116747',
                  $options
                  );                   
...
{
    return Cache::lock('fruits_pusher-switch-global-lock', 5)->block(3, function () use ($pusher_id) {
        ...
        try {
            DB::transaction(function () use ($current) {
                ...
                $nextPusher = PusherKey::where('used_for',2)->where('pusher_status', 0)
