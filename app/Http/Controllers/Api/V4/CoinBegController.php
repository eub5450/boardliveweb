...
class CoinBegController extends Controller
{
    ...
    //   $coinBeg_run = CoinBeg::where('host_id', $host_id)->where('channelName', $channelName)->where('created_at', '>=', now()->subMinutes(1))  // Fetch records from the last 1 min
    //     ->first();
    //     if($coinBeg_run){
    //          return response()->json([
    ...
    //             'ul.pin', 'ul.audio_brd_design', 'u.host_badge', 'ul.sdk'
    //         ])
    //         ->first();
    ...
    //   self::RealTime($websocket_call, $roomName, $channelName);
    ...
    //                     $next_comment_ref->set($beg_comment);
      ...
      if ($token !== "0411f0028cfb768b3a3d96ac3aa37dw3e5") {
       ...
       $coinBegReceived = CoinBegRecived::firstOrCreate(
        ...
        sleep(12);
         ...
         if($coinBeg->click==0){
         $this->CoinBegResult($userId, $beg_id, $channelName);
         }
        }
        return response()->json([
                'success' => true,
  ...
  private function CoinBegResult($userId, $beg_id, $channelName)
{
//     $coinBeg = CoinBeg::find($beg_id);
...
//     $now_balance = floor($amount - ($amount * 0.10));
 ...
 private function RealTime($response, $roomName, $channelName)
    ...
    try {
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'http://bdlive.org/api/bd_recieve-three',
        ...
        if (curl_errno($curl)) {
            ...
            info('Hit From Coin Beg: ' . $http_code);
        }
