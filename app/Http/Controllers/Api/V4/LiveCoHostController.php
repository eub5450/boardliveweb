...
class LiveCoHostController extends Controller
{
  ...
  public function AudioCallMuteFirebase(Request $request)
  {
  	$access_token = $request->access_token;
        ...
        $response = array();
        if($access_token=="0411f0028cfb768b3a3d96ac3aa37dw3e5"){
            $data=LiveCall::where('channelName',$channelName)->where('co_host_id',$user_id)->where('status','Accept')->first();
          if($data){
          if($data->mute==0){
          $data->mute=1;
          }else{
          ...
          $audio_call=array();
        	$row['channelName'] = $channelName;
    		$row['co_host_id'] = strval($user_id);
            $row['set_no'] = strval($set_no);
          	$row['status'] = strval($status);
    		$push_count_ref = $this->database->getReference('audio_call_mute/' . $channelName .'/'. $user_id);
    		$push_count_ref->set($row);
    		
    		
    		
          array_push($response,array('message'=>'Mute Status Send ','code'=>'200'));
            return json_encode($response,JSON_UNESCAPED_UNICODE);
        }else{
            array_push($response,array('message'=>'Unauthorized access_token','code'=>'401'));
            return json_encode($response,JSON_UNESCAPED_UNICODE);
