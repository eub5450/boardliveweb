                    ...
                    if ($sander->balance >=$value) {
                       ...
                       if($value==500){
                        ...
                        $game_balance=0;
                         ...
                         $lucky_balance=LuckyGiftSetting::find(1);
                        $gift = new Gift;
                        $gift->sander_id = $user_id;
                        $gift->reciever_id = $row['receiverId'];
                        $gift->name = $gift_name;
                      
                        $gift->value = $host_amount;
                        $gift->channelName=$channelName;
                        $gift->date = Carbon\Carbon::now();
                         if($gift->save()){
                              $sander->balance -= $value;
                              $sander->save();
                             $lucky_balance->balance+=$game_balance;
                             $lucky_balance->save();
                         }
                         if($reward!=0 && $lucky_balance->balance >$reward){
                             $lucky_balance->balance-=$reward;
                             $lucky_balance->save();
                             $sander->balance += $reward;
                              $sander->save();
                         }else{
                           $reward=0;  
                         }
...
$next_level_amount=50000;
              ...
              app(\App\Services\V4\RealtimePusherService::class)
                  ->trigger($pusher_response, 'video_call_host_list', $channelName);
               if($reward!=0){
