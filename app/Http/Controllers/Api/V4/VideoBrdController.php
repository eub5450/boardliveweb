...
class VideoBrdController extends Controller
{
    ...
    private function v4DailyRewardSettings(): array
    {
        $thresholdMinutes = $this->rewardSettingInt('video_reward_v4_threshold_minutes', 60);

        return [
            'enabled' => $this->rewardSettingBool('video_reward_v4_enabled', true),
            'threshold_minutes' => $thresholdMinutes,
            'threshold_seconds' => $thresholdMinutes * 60,
            'amount' => $this->rewardSettingInt('video_reward_v4_amount', 4000),
            'sender_id' => $this->rewardSettingInt('video_reward_v4_sender_id', 11162),
            'gift_name' => $this->rewardSettingString('video_reward_v4_gift_name', 'reward.svga'),
        ];
    }
    ...
    public function VideoBrdDayTimeRequest(Request $request)
    {
        ...
        if(LiveStreamHelper::validateAccessToken($token)){
            ...
            if ((string) $brd_type === '2') {
                ...
                $rewardSettings = $this->v4DailyRewardSettings();

                if ($rewardSettings['enabled'] && $totalDurationInSeconds >= $rewardSettings['threshold_seconds']) {
                    $existingReward = Gift::where('reciever_id', $id)
                        ->whereDate('date', Carbon\Carbon::today())
                        ->where('sander_id', $rewardSettings['sender_id'])
