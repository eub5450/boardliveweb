...
class PortalController extends Controller
{
    ...
    private function portalSettingValue(string $key, int $default): int
    {
    ...
    {
        return $this->portalSettingValue('portal_min_transfer_amount', 50000);
    }

    private function portalToPortalAmountRules(): array
    {
        $minimumAmount = $this->portalSettingValue('portal_to_portal_min_amount', 500000);
        $maximumAmount = $this->portalSettingValue('portal_to_portal_max_amount', 10000000);
        $stepAmount = $this->portalSettingValue('portal_to_portal_step_amount', 100000);
    ...
    private function isValidPortalToPortalAmount(int $amount, array $rules): bool
    {
        if ($amount < $rules['min'] || $amount > $rules['max']) {
            return false;
        }

        return (($amount - $rules['min']) % $rules['step']) === 0;
    }
    ...
    public function ProtalTransfer(Request $request)
    {
        ...
        $amountRules = $this->portalToPortalAmountRules();

        if (!$this->isValidPortalToPortalAmount($amount, $amountRules)) {
            return response()->json([
                'message' => 'Allowed Amount ' . $amountRules['min'] . ' to ' . $amountRules['max'] . ' step ' . $amountRules['step'],
                'code' => 401
            ]);
        }
