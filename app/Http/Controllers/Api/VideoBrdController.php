...
{
    private function rewardSettingInt(string $key, int $default): int
    {
    ...
    private function rewardSettingString(string $key, string $default): string
    {
        $setting = Setting::find(1);
    ...
    private function rewardSettingBool(string $key, bool $default): bool
    {
        $setting = Setting::find(1);

        return (int) data_get($setting, $key, $default ? 1 : 0) === 1;
    }

    private function legacyDailyRewardSettings(): array
    {
        $thresholdMinutes = $this->rewardSettingInt('video_reward_legacy_threshold_minutes', 50);

        return [
            'enabled' => $this->rewardSettingBool('video_reward_legacy_enabled', true),
            'threshold_minutes' => $thresholdMinutes,
            'threshold_seconds' => $thresholdMinutes * 60,
            'amount' => $this->rewardSettingInt('video_reward_legacy_amount', 5000),
            'sender_id' => $this->rewardSettingInt('video_reward_legacy_sender_id', 11162),
            'gift_name' => $this->rewardSettingString('video_reward_legacy_gift_name', 'reward.svga'),
            'block_enabled' => $this->rewardSettingBool('video_reward_legacy_block_enabled', true),
            'block_start' => $this->rewardSettingString('video_reward_legacy_block_start', '05:00:00'),
            'block_end' => $this->rewardSettingString('video_reward_legacy_block_end', '11:00:00'),
        ];
    }
   ...
   public function VideoBrdDayTimeRequest(Request $request)
    {
       ...
       $sum_date_time = DayTime::where('user_id', $id)
        ...
        $rewardSettings = $this->legacyDailyRewardSettings();

       
       $currentTime = Carbon\Carbon::now();
       $blockStart = Carbon\Carbon::today()->setTimeFromTimeString($rewardSettings['block_start']);
       $blockEnd = Carbon\Carbon::today()->setTimeFromTimeString($rewardSettings['block_end']);
    
     if ((!$rewardSettings['block_enabled'] || !$currentTime->between($blockStart, $blockEnd)) && $rewardSettings['enabled']) {
         if ($totalDurationInSeconds >= $rewardSettings['threshold_seconds']) {
               $existingReward = Gift::where('reciever_id', $id)
                                 ->whereDate('date', Carbon\Carbon::today())->where('sander_id',$rewardSettings['sender_id'])
                                 ->where('name', $rewardSettings['gift_name'])
                                 ->where('value', $rewardSettings['amount'])
                                 ->first();

                 if (!$existingReward) { 
                         $value = $rewardSettings['amount'];
                         $sander_user = User::find($rewardSettings['sender_id']);
                         $gift = new Gift;
                         $gift->sander_id = $sander_user->id;
                         $gift->reciever_id = $id;
                         $gift->name = $rewardSettings['gift_name'];
                         $gift->value = $value;
                         $gift->channelName=$channelName;
                         $gift->date = Carbon\Carbon::now();
                         if($gift->save()){
                             ...
                             $commnet_message = "{$reciever->name} Got {$value} Coin Reward From Broad Live For {$rewardSettings['threshold_minutes']} Minute Daily Live Completion";
                            
                                     $data=new Comment;
                                     $data->user_id=$rewardSettings['sender_id'];
                                     $data->channelName=$channelName;
                ...
                // info('give_time_reward_amount: ' . $reward_amount. ' user_ID ' .$id. ' day_time ' .$totalDurationInSeconds. ' date ' .\Carbon\Carbon::now(config('app.timezone', 'Europe/London'))->toDateString());
             }
            // info('atteped_reward_amount: ' . $reward_amount. ' user_ID ' .$id. ' day_time ' .$totalDurationInSeconds. ' date ' .\Carbon\Carbon::now(config('app.timezone', 'Europe/London'))->toDateString());
