...
class TeenPattiController extends Controller
{
    public function Index(Request $request)
    {
       ...
       $is_another_id_lock=FortuneLock::where('imei_number',$user->imei_number)->where('type',0)->first();
        ...
        if($user->id==11142){
                    return view('game.teenpatti.index2',compact('authtoken','authkey'));

        }else{
        $fortunesetting = TeenPattiSetting::select('pusher_id')->first();
        return view('game.teenpatti.index',compact('authtoken','authkey','fortunesetting'));
  ...
  public function BidInsart(Request $request)
{
    ...
    if (TeenPattiTray::where('tray_id', $trayId)->exists()) {
        DB::transaction(function () use ($amount, $loginUser, $trayId, $boardName) {
            ...
            $fortunesetting = TeenPattiSetting::select('app_id_pusher','key_pusher','secret_pusher','cluster_pusher','pusher_id')->first();      
        ...
        try {  
              $pusher->trigger('users_amount_name', 'users_amount_event', ['bord_name' => $boardName, 'bord_amount' => $amount]); 
                    ...
                    if ($switched) {
                        ...
                        $newSetting = TeenPattiSetting::select('app_id_pusher','key_pusher','secret_pusher','cluster_pusher','pusher_id')->first();;
         ...
         $check_lock_list=FortuneLock::where('user_id',$loginUser->id)->where('type',0)->first();
            ...
            if ($existingGameStart) {
                ...
                $existingGameStart->win_balance =intval(round($existingGameStart->amount  * 2.9));
                ...
                $gameStart->win_balance =intval(round($amount * 2.9));
    ...
    public function GameWinner(Request $request)
    {
        ...
        Auth::login($user);
            if($request->has('tray_id')){
                ...
                if (!$findallusers || empty($findallusers->cards)) {
                    return response()->json(['data' => [], 'st' => false, 'message' => 'Winner data not ready']);
                }
                $cards = json_decode($findallusers->cards, true);
                if (!is_array($cards)) {
                    return response()->json(['data' => [], 'st' => false, 'message' => 'Winner data invalid']);
                }
                $arraydata = array($cards);
    ...
    public function WinnerDecission($tray_id)
    {
        ...
        if ($result) {
               $fortunesetting = TeenPattiSetting::first();             
                    ...
                    $apple_need_serve = $apple*2.9;
                    $watermelon_need_serve= $watermelon*2.9;
                    $lemon_need_serve= $lemon*2.9;
...
{
    return Cache::lock('teenpatti_pusher-switch-global-lock', 5)->block(3, function () use ($pusher_id) {
        ...
        try {
            DB::transaction(function () use ($current) {
                ...
                $nextPusher = PusherKey::where('used_for',3)->where('pusher_status', 0)
                ...
                TeenPattiSetting::where('id', 1)->update([
